...
class Game(object):
    def __init__(self):
        ...
        self.board = [[0 for y in range(self.column)] for x in range(self.row)]
        self.is_start = False
        self.is_black = True
        self.last_p = None
        ...
        self.l_info = Label(self.f_header, text="Waiting", bg=self.header_bg, font=("楷体", 18, "bold"), fg="white")
    ...
    # 用last_p来标识上一步的位置。先用网格覆盖掉棋子，操作相应的变量，board[x][y]要置空，只能悔一次棋
    def bf_regret(self):
        if not self.last_p:
            showinfo("提示", "现在不能悔棋")
            return
    ...
    def cf_board(self, e):
        ...
        if distance > self.step * 0.95 or self.board[x][y] != 0 or not self.is_start:
            return
        # 此时棋子的颜色，和board中该棋子的标识。
    ...
    def is_win(self, x, y, tag):
        # 获取斜方向的列表
        def direction(i, j, di, dj, row, column, board):
            temp = []
